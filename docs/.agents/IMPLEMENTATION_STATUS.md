...
| 0018 | Runtime Variants Architecture | PARTIAL (architecture ACCEPTED; Phase 0 primitive IMPLEMENTED) | ADR accepted (architecture only) 2026-06-11 (Task 26). Defines `RuntimeVariant` as an *infrastructure descriptor* (Runtime × Checkpoint), distinct from `VoiceVariant`; amends ADR-0016's forbidden-pattern clause. **Phase 0 (additive, non-wired) IMPLEMENTED:** `RuntimeVariantDescriptor` + `RuntimeCheckpoint` (`app/services/runtime_types.py`); variant index + `list_variants_for_runtime`/`get_variant` + `variants/*.json` loader (`app/services/runtime_registry.py`); `tests/test_runtime_variant_descriptor.py` (13 tests; 98 passing with existing registry/descriptor tests). No resolution/lifecycle/API/UI change yet — Phases 1–6 PLANNED. See `docs/.agents/SPECS/FEATURES/runtime-variants/` + `docs/.agents/IMPLEMENTATION/PLANS/2026-06-11-runtime-variants-migration.md`. |
