...
| 0018 | Runtime Variants Architecture | PARTIAL (Phases 0,1,3 IMPLEMENTED; 2 image work + 4,5,6-tail PLANNED) | ADR accepted (architecture only) 2026-06-11 (Task 26); **wired in Task 27** (2026-06-11). Phase 0 primitive (`RuntimeVariantDescriptor`/`RuntimeCheckpoint`, variant index, `variants/*.json` loader). **Phase 1 (resolution) IMPLEMENTED:** `RuntimeRegistry.select_variant()` + additive `RuntimeResolution.runtime_variant_id` (implicit-base = `None`, byte-identical) — `runtime_registry.py`, `runtime_manager.py`; `tests/test_runtime_variant_resolution.py`. **Phase 3 (presentation) IMPLEMENTED:** public-safe `variants[]` on `/models/with-runtimes` (`runtime_api.py`) + frontend Variants section/badges/import dialog (`frontend/src/components/models/VariantsSection.tsx`). Concrete `variants/base.json` ship for all 3 runtimes. Download+register (Phase 6 tail), multi-checkpoint runtime service (Phase 4), directory consolidation + shared base image (Phase 2), marketplace (Phase 5) PLANNED. See `docs/.agents/SPECS/FEATURES/runtime-variants/`. |
| 0019 | Variant Trust Tiers & Community Imports | PARTIAL (trust + validate IMPLEMENTED; download/register PLANNED) | ADR accepted 2026-06-11 (Task 27; **3 tiers Task 27.1**); extends 0018. **IMPLEMENTED:** `trust` (verified\|community\|**private**, default verified) + `source_url` on `RuntimeVariantMetadata` (`runtime_types.py`); validate-only import `validate_variant_import()` derives trust from provenance — public→community, local→private (`app/services/runtime_variant_import.py`) + `POST /runtimes/{id}/variants/validate-import` (`runtime_api.py`); frontend tri-state trust badges (✓/⚠/🔒) + import dialog. Gates: capability vocabulary + ceiling, provider/family match, format — declared-and-checked (ADR-0003). Trust is provenance only (never affects resolution). `tests/test_runtime_variant_import.py`, `test_runtime_variant_resolution.py`, composed-view tests. Download to `/data/runtime-variants/` + register (two-root loader) PLANNED — see `docs/.agents/VALIDATION/RESEARCH/task-27.1-runtime-registry-2.0-review.md`. |
...
| XTTS v2 adapter + runtime | IMPLEMENTED (arch) / PARTIAL (provider) | **Task 30 / ADR-0021.** `model_adapters/xtts_adapter.py` (`reference_sample`, runtime-routed via `HTTPTransport`, no in-process exec; sibling of F5), wired in `model_wiring.py` (`provider="xtts"`); `xtts-v2` catalog entry (`model_catalog.py`, 17 languages, CPML non-commercial, CE-disabled); `runtime-registry/xtts-v2/` (descriptor + `peakvox/xtts-runtime` server + `variants/base.json`, auto-discovered). Capabilities = F5's set (tts/cloning/multilingual/reference_audio/voice_optional). **CPU-capable** (`gpu="optional"`, `server.py` CPU fallback — the divergence from F5). Architecture-validated: `tests/test_xtts_adapter.py` (11) + `runtime-registry/xtts-v2/tests/` (48: server contract, CPU fallback, voice-optional, concurrency, descriptor, base variant). Real container audio gen on GPU/CPU is provider-validation (pending) — see `VALIDATION/PROVIDER_VALIDATIONS/task-30-xtts-validation.md` |
