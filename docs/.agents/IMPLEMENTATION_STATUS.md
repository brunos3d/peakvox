...
| 0019 | Variant Trust Tiers & Community Imports | PARTIAL (trust + validate IMPLEMENTED; download/register PLANNED) | ADR accepted 2026-06-11 (Task 27); extends 0018. **IMPLEMENTED:** `trust` (verified\|community, default verified) + `source_url` on `RuntimeVariantMetadata` (`runtime_types.py`); validate-only community import `validate_variant_import()` (`app/services/runtime_variant_import.py`) + `POST /runtimes/{id}/variants/validate-import` (`runtime_api.py`); frontend trust badges + import dialog. Gates: capability vocabulary + ceiling, provider/family match, format — declared-and-checked (ADR-0003). `tests/test_runtime_variant_import.py` + composed-view/endpoint tests in `test_api_models_with_runtimes.py`. Download to weights cache + descriptor registration PLANNED (runs inside runtime container). See `docs/.agents/VALIDATION/RESEARCH/task-27-model-ecosystem-findings.md`. |
