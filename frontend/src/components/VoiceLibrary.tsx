...
import { VoiceProfileAudioInput, AudioInputResult } from "@/components/VoiceProfileAudioInput"
import { createVoice, updateVoice as updateVoiceApi, deleteVoice, getVoiceAudioUrl } from "@/lib/api"
...
export function VoiceLibrary() {
  ...
  // Create form state
  const [createForm, setCreateForm] = useState({ name: "", transcript: "", language: "Auto" })
  const [createAudio, setCreateAudio] = useState<AudioInputResult | null>(null)

  // Edit form state
  const [editForm, setEditForm] = useState({ name: "", transcript: "", language: "Auto" })
      ...
      <div className="flex items-center justify-between">
        ...
        <Dialog
          ...
          onOpenChange={(open) => {
            ...
            if (!open) { setCreateForm({ name: "", transcript: "", language: "Auto" }); setCreateAudio(null) }
          }}
        >
          ...
          <DialogContent className="sm:max-w-xl">
            <DialogHeader>
              ...
              <DialogDescription>
                Upload or record a voice sample. Files longer than 10s will open a crop editor.
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <Label>Reference Transcript <span className="text-muted-foreground font-normal">(optional)</span></Label>
                <Textarea
                  placeholder="Transcript of the reference audio — improves cloning accuracy."
                  value={createForm.transcript}
                  onChange={(e) => setCreateForm({ ...createForm, transcript: e.target.value })}
                  rows={2}
                />
              </div>
              <div className="space-y-2">
        ...
        <Dialog
          ...
          onOpenChange={(open) => {
            setIsEditOpen(open)
            if (!open) { setEditingProfile(null); setEditAudio(null) }
          }}
        >
          <DialogContent className="sm:max-w-xl">
            <DialogHeader>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <Label>Replace Audio <span className="text-muted-foreground font-normal">(optional)</span></Label>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    <Button
                      ...
                      size="icon"
                      className="h-7 w-7"
                      onClick={(e) => { e.stopPropagation(); previewAudio(profile) }}
                    >
