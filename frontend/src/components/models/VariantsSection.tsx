...
import type { ComposedRuntimeVariant, RuntimeVariantTrust, VariantImportValidation } from "@/types"
    ...
    <div className="rounded-md border border-border bg-surface-2 p-3 space-y-2">
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0 space-y-0.5">
          ...
          <p className="text-[10px] text-muted-foreground font-mono">{variant.id}</p>
        </div>
...
// ---------------------------------------------------------------------------
function ImportVariantDialog({ runtimeId }: { runtimeId: string }) {
  ...
  async function onValidate() {
    ...
    try {
      ...
      const res = await validateVariantImport(runtimeId, {
        url,
        declared_provider: provider.trim() || undefined,
        declared_capabilities: caps.length ? caps : undefined,
      })
      setResult(res)
    } catch (e) {
    ...
    >
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Attach a Hugging Face checkpoint to this runtime as a new variant — no new image,
            no rebuild. This step <strong>validates compatibility only</strong>; download and
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <Label htmlFor="variant-provider">Declared provider</Label>
              <Input
                id="variant-provider"
                placeholder="f5-tts"
                value={provider}
            ...
            <div className="space-y-1.5">
              <Label htmlFor="variant-caps">Capabilities</Label>
              <Input
                id="variant-caps"
                placeholder="tts, voice_cloning"
                value={capabilities}
          ...
          {result && (
            ...
            >
              ...
              <p className="text-xs text-muted-foreground">
                Repo <span className="font-mono">{result.repo_id}</span> → variant{" "}
              ...
              {result.compatible && (
                <p className="flex items-center gap-1 text-[11px] text-muted-foreground">
                  <Download className="h-3 w-3" /> Download &amp; register is coming in a
                  follow-up — the checkpoint passes the compatibility gates.
                </p>
              )}
            </div>
          )}
        ...
        <DialogFooter>
          ...
          <Button size="sm" onClick={onValidate} disabled={pending || !url.trim()}>
    ...
    <div className="space-y-2">
      <div className="flex items-center justify-between gap-2">
        <p className="text-caption uppercase tracking-wide flex items-center gap-1.5">
          ...
          {list.length > 0 && (
            <span className="text-muted-foreground normal-case">({list.length})</span>
          )}
        </p>
        <ImportVariantDialog runtimeId={runtimeId} />
      </div>

      {list.length === 0 ? (
        <p className="text-xs text-muted-foreground">
          Single base runtime — no additional variants installed. Import a Hugging Face
          checkpoint to add a specialization (e.g. a language or style) without a new image.
        </p>
      ) : (
        <div className="space-y-2">
          {list.map((v) => (
